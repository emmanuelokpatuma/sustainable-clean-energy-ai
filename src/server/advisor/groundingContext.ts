...
export interface GroundingContextInput {
  ...
  /** Not available until Phase 8 exists to produce real recommendations. */
  recommendations?: { title: string; explanation: string; priority: number }[] | null;
}
...
function renderRecommendationsSection(
  ...
  if (!recommendations || recommendations.length === 0) {
    return "RECOMMENDATIONS: not available yet (Action Plans are built in a later phase). Do not invent a prioritised action list — you may still discuss general opportunities visible in the other sections above.";
  }
  const lines = ["RECOMMENDATIONS (in priority order):"];
  for (const r of recommendations) {
    lines.push(`- [Priority ${r.priority}] ${r.title}: ${r.explanation}`);
  }
  return lines.join("\n");
}
