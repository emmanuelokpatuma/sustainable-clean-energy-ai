...
describe("renderGroundingContext — Recommendations section", () => {
  it("lists provided recommendations in priority order with their impact and next step", () => {
    const text = renderGroundingContext({
      recommendations: [
        {
          id: "investigate-solar",
          title: "Investigate solar suitability",
          explanation: "High suitability at this location.",
    ...
    expect(text).toContain("[Priority 1] Investigate solar suitability");
    expect(text).toContain("525 kg CO2/year");
    expect(text).toContain("£343/year");
    expect(text).toContain("Get a quote from an MCS-certified installer.");
  });

  it("states no specific figure when an action has no quantified impact", () => {
    const text = renderGroundingContext({
      recommendations: [
        {
          id: "shift-flexible-use",
          title: "Shift flexible electricity use",
          explanation: "A cleaner window is forecast later today.",
          impactCategory: "carbon",
          ...
          suggestedNextStep: "Check Energy Now before running flexible appliances.",
          priority: 1,
        },
      ],
    });
    expect(text).toContain("no specific figure calculated");
  });

  it("tells the model these are generated by rules, not something it should invent", () => {
    const text = renderGroundingContext({ recommendations: [] });
    expect(text).toContain("Do not invent a prioritised action list");
  });
});
